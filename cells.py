...
def startColor():
	"""Gives parent cell initial colour from text file"""
	global colours
	if len(colours) == 0: colours = globals.colourList()
	pos = random.randint(0,(len(colours)-1))
	return pop.colours(pos)

def genRandomColour(rgbTuple):
	"""creates a random colour based on provided tuple"""
	rgbList   = list(rgbTuple)
	amount = [0,0,0]
	amounts= []
	for value in amount:
		while value == 0: value = random.randint(-20,20)
		amounts.append(value)

	#decides whether or not each value changes
	#0-2: single colour change	(0-r,   1-g,   2-b)
	#3-5: double colour change	(3-rg,  4-rb,  5-gb)
	#6-7: all or nothing		(6-rgb, 7-none)
	mutate = random.randint(0,7)
	
	#changes single colour
	...
	#changes two colours
		...
		if (mutate == 3) or  (mutate == 5): rgbList[1] += amounts[1]
		if (mutate == 4) or  (mutate == 5): rgbList[2] += amounts[2]
		
	#changes all colours
	elif   (mutate == 6):
		for number in range(2): rgbList[number] += amounts[number]
	
	#makes sure colour is valid
...
class Cell:
	...
	def __init__(self, x, y,  mass=0.3, energy=0.1, x_vel=0.0, y_vel=0.0, Phenotype=[default_emRatio, default_div_energy, default_div_mass, default_color, default_walk_force, default_density , default_mutation_chance]):
		...
		if self.colour == None:   self.colour = cells.startColour()
		else:			  self.colour = cells.genRandomColour(self.colour)
	...
	def determine_phenotype(self):
		...
		if random.uniform(0,100)>mutation_chance:
			...
			newphenotype.append(self.phenotype[3])
